...
const debugObject = {
    createSphere: () => {
        ...
        createSphere(Math.random() * 0.8, { x, y: 3, z });
    },
    createBox: () => {
        const x = (Math.random() - 0.5) * 6;
        const z = (Math.random() - 0.5) * 6;
...
const playHitSound = (collision) => {
    ...
    if (impactStrength > 1.5) {
        hitSound.volume = Math.min(impactStrength / 7.5, 1);
...
const createBox = (w, h, d, position) => {
    ...
    // const shape = new CANNON.Box(w / 2, h / 2, d / 2);
    const body = new CANNON.Body({
        mass: 1,
        position: new CANNON.Vec3(0, 3, 0),
        shape: shape,
...
const tick = () => {
    ...
    // Fixed time step, time since last, it to catch up
